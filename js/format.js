...
function format(ex, acc=3, max=9, type="mixed_sc") {
    ex = E(ex)
    neg = ex.lt(0)?"-":""
...
function formatTime(ex,acc=2,type="s") {
    ex = E(ex)
    if (ex.gte(86400)) return format(ex.div(86400).floor(),0,12,"sc")+" day(-s) "+formatTime(ex.mod(86400),acc,'d')
...
function formatPercent(ex,acc=3) { ex = E(ex); return format(ex.mul(100),acc)+"%" }
